...
export default function ItemPanel({ showButton } : { showButton: boolean }) {
  ...
  const onSubmit = useCallback(async (data: z.infer<typeof formSchema>) => {
    ...
    if (data.image) {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/itemType/image/${item.id}`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token!.value}`,
        },
  ...
  return (
    <>
      <div className="flex justify-end pt-2 pb-2 bg-gradient-to-r from-rose-500 to-red-700 min-h-[72px]">
        {showButton && 
        <Button variant="destructive" className="mr-2 my-2" onClick={(e) => setEditDialogOpen(true)}>
          Bæta við vöru
        </Button>
        }
      </div>
